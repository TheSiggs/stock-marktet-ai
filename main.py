...
while True:
    time.sleep(10)
    new_ohlc = \
        kraken.get_ohlc_data({'pair': 'XXBTZUSD', 'interval': interval, 'since': ohlc_formatted[-1]['Unix Timestamp']})[
            'result'][
            'XXBTZUSD']
    for data in new_ohlc:
        ...
        print(colored(new_entry, 'red' if new_entry['Description'] == 'Bearish' else 'green'))
